class Assignment2_Python(object):
    ...
    def IsMagic(num: int, max_iterations: int) -> int:
        ...
        for i in range (0, max_iterations):
            if(temp % 2 == 0):
                temp = Assignment2_Python.even(temp)
            else:
                temp = Assignment2_Python.odd(temp)
            if(temp <=0):
                return num/-1
            if temp == 1:
                return itr
    ...
    def TestRange(start: int, stop: int, max_iterations: int) -> int:
        for i in range(start, stop + 1):
            if(Assignment2_Python.IsMagic(i, max_iterations) == i/-1):
                return i/-1
        return Assignment2_Python.IsMagic(stop, max_iterations)
